    ...
    return openai.chat.completions.create({
      messages: [
        { role: 'system', content: 'you are a novelist.' },
        {
          ...
          // content: `다음 키워드로 소설을 써줘. 인물:[${characters !== "" ? `${characters}` : "정해지지 않음"}], 사건: [${events !== "" ? `${events}` : "정해지지 않음"}], 배경: [${backgrounds !== "" ? `${backgrounds}` : "정해지지 않음"}]. 인물들의 대사도 포함해줘. 그리고 대사는 문장을 분리해줘. 소설 내용만 얘기하고, 제목이랑 설명글은 적지 마. 닫힌 결말로 이야기를 짓고 2000자 이상 적어줘.`
